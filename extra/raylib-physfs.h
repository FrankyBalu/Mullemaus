...
RAYLIB_PHYSFS_DEF bool
InitPhysFSEx(const char *newDir, const char *mountPoint);  // Initialize the PhysFS file system with a mount point.
RAYLIB_PHYSFS_DEF bool ClosePhysFS();                                             // Close the PhysFS file system
RAYLIB_PHYSFS_DEF bool
IsPhysFSReady();                                           // Check if PhysFS has been initialized successfully
RAYLIB_PHYSFS_DEF bool
MountPhysFS(const char *newDir, const char *mountPoint);   // Mount the given directory or archive as a mount point
RAYLIB_PHYSFS_DEF bool MountPhysFSFromMemory(const unsigned char *fileData, int dataSize, const char *newDir,
...
RAYLIB_PHYSFS_DEF bool
FileExistsInPhysFS(const char *fileName);                  // Check if the given file exists in PhysFS
RAYLIB_PHYSFS_DEF bool
DirectoryExistsInPhysFS(const char *dirPath);              // Check if the given directory exists in PhysFS
RAYLIB_PHYSFS_DEF unsigned char *LoadFileDataFromPhysFS(const char *fileName,
                                                        unsigned int *bytesRead);  // Load a data buffer from PhysFS (memory should be freed)
RAYLIB_PHYSFS_DEF char *
LoadFileTextFromPhysFS(const char *fileName);             // Load text from a file (memory should be freed)
RAYLIB_PHYSFS_DEF bool SetPhysFSWriteDirectory(
        const char *newDir);               // Set the base directory where PhysFS should write files to (defaults to the current working directory)
RAYLIB_PHYSFS_DEF bool SaveFileDataToPhysFS(const char *fileName, void *data,
...
RAYLIB_PHYSFS_DEF FilePathList
LoadDirectoryFilesFromPhysFS(const char *dirPath);  // Get filenames in a directory path (memory should be freed)
RAYLIB_PHYSFS_DEF long
GetFileModTimeFromPhysFS(const char *fileName);            // Get file modification time (last write time) from PhysFS
...
RAYLIB_PHYSFS_DEF Font
LoadFontFromPhysFS(const char *fileName, int fontSize, int *fontChars, int charsCount);  // Load a font from PhysFS
RAYLIB_PHYSFS_DEF Shader
LoadShaderFromPhysFS(const char *vsFileName, const char *fsFileName);  // Load shader from PhysFS
RAYLIB_PHYSFS_DEF void
SetPhysFSCallbacks();                                      // Set the raylib file loader/saver callbacks to use PhysFS
RAYLIB_PHYSFS_DEF const char *GetPerfDirectory(const char *organization,
...
unsigned char *LoadFileDataFromPhysFS(const char *fileName, unsigned int *bytesRead) {
    ...
    int size = PHYSFS_fileLength((PHYSFS_File *) handle);
    if (size == -1) {
        *bytesRead = 0;
    ...
    int read = PHYSFS_readBytes((PHYSFS_File *) handle, buffer, size);
    if (read < 0) {
        *bytesRead = 0;
        MemFree(buffer);
        PHYSFS_close((PHYSFS_File *) handle);
...
bool SaveFileDataToPhysFS(const char *fileName, void *data, unsigned int bytesToWrite) {
    ...
    if (PHYSFS_writeBytes((PHYSFS_File *) handle, data, bytesToWrite) < 0) {
        PHYSFS_close((PHYSFS_File *) handle);
        TracePhysFSError(fileName);
        return false;
    }
