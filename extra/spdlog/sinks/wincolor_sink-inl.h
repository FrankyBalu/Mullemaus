...
// print a range of formatted message to console
        ...
        wincolor_sink<ConsoleMutex>::print_range_(const memory_buf_t &formatted, size_t start, size_t end) {
            if (end > start) {
                ...
                auto ignored = ::WriteConsoleA(static_cast<HANDLE>(out_handle_), formatted.data() + start, size,
                                               nullptr, nullptr);
                (void) (ignored);
            }
        }
        ...
        void SPDLOG_INLINE wincolor_sink<ConsoleMutex>::write_to_file_(const memory_buf_t &formatted) {
            ...
            auto ignored = ::WriteFile(static_cast<HANDLE>(out_handle_), formatted.data(), size, &bytes_written,
                                       nullptr);
            (void) (ignored);
        }
